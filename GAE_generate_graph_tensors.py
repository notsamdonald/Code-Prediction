...
if __name__ == "__main__":
    ...
    # Using the final 10% of data to train and validate the GAE TODO - confirm how its validated/tested
    ratio = 0.1
    train_len, val_len, test_len = len(dataset['train']), len(dataset['validation']), len(dataset['test'])
    train_split, val_split, test_split = split_index(train_len), split_index(val_len), split_index(test_len)
    ...
    with open('GAE_graph_tensors.pkl', 'wb') as handle:
